...
void Viewer_Run(Viewer* pViewer)
{
    ...
    while(1)
    {
        ...
        pViewer->mpMapDrawer->GetCurrentOpenGLCameraMatrix(Twc);
        ...
        pViewer->mpMapDrawer->DrawCurrentCamera(Twc);
        if(menuShowKeyFrames || menuShowGraph)
            pViewer->mpMapDrawer->DrawKeyFrames(menuShowKeyFrames,menuShowGraph);
        if(menuShowPoints)
            pViewer->mpMapDrawer->DrawMapPoints();

        pangolin::FinishFrame();

        cv::Mat im = pViewer->mpFrameDrawer->DrawFrame();
